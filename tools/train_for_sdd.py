...
def main():
    ...
    do_train, _, inference = build_engine(cfg)
    ...
    if cfg.SOLVER.scheduler == 'exp':
        ...
        lr_scheduler = CosineAnnealingWarmbootingLR(optimizer, epochs=cfg.SOLVER.MAX_EPOCH, steps=cawb_steps, step_scale=0.8,
                                                 lf=lf, batchs=len(train_dataloader))  # FIXME set last_epoch to recover training
    else:
        lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[25, 40], gamma=0.2)
                                                        
    print('Schedulers built!')
    best_score = 999  # FIXME set best_score to recover training
        ...
        if (epoch+1) % 1 == 0:
            ...
            for i in range(times):
                eval_results = inference(cfg, 0, model, test_dataloader, cfg.DEVICE, logger=logger, eval_kde_nll=False,
                                         test_mode=False)
                if i == 0:
                    result['ADE'] = eval_results['ADE']
                    result['FDE'] = eval_results['FDE']
                else:
                    result['ADE'] += eval_results['ADE']
                    result['FDE'] += eval_results['FDE']
            score = (result['ADE'] + result['FDE'])/times
            if score < best_score:
