...
class CosineAnnealingWarmbootingLR:
    ...
    def __init__(self, optimizer, epochs=0, eta_min=1e-6, steps=[], step_scale=0.8, lf=None, batchs=0, warmup_epoch=0,
...
def train(opt):
    ...
    cawb_steps = lambda x: (1 - pow(2, x)) / (1 - 2) * 5
    print(cawb_steps)
    # lf = lambda x, y=opt.epochs: (1.0 - (x / y)) * 0.9 + 0.1
    scheduler = CosineAnnealingWarmbootingLR(optimizer, epochs=opt.epochs, steps=opt.cawb_steps, step_scale=0.8,
    ...
    for i in range(opt.epochs):
        for b in range(len(data)):
            lr = scheduler.step_batch()  # before the backward
            # training
            # loss
            # backward
        scheduler.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cawb_steps', nargs='+', type=int, default=[5, 15, 35, 75, 155],
