...
def lam(device, start, finish, center_step, steps_lo_to_hi, step):
    return start + (finish - start)*torch.sigmoid((torch.tensor(float(step), device=device) - center_step) * (1./steps_lo_to_hi))
