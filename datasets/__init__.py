...
import pdb
_DATA_LAYERS = {
    'eth_STGlow': ETHUCYDataset,
    'hotel_STGlow': ETHUCYDataset,
    'univ_STGlow': ETHUCYDataset,
    'zara1_STGlow': ETHUCYDataset,
    'zara2_STGlow': ETHUCYDataset,
    'sdd_STGlow': SDDDataset,
}
...
def collate_dict(batch):
    ...
    for key in all_keys:
        ...
        if elem[key] is None:
            ...
            # collate_batch[key] = list(torch.stack(b[key], dim=0) for b in batch)
            collate_batch[key] = torch.cat([torch.stack(b[key], dim=0) for b in batch], 0)
        else:
