...
def get_node_timestep_data(env, scene, t, node, state, pred_state,
    ...
    if hyperparams['edge_encoding']:
        ...
        neighbors_data = list()  # 7.18
        neighbors_data_st = list()  # 7.18
        neighbors_data.append(x_t)  # 7.18
        neighbors_data_st.append(x_st_t)  # 7.18
        neighbors_data_lower_upper = dict()
        neighbors_edge_value = dict()
        neighbors_future = dict()
        for edge_type in edge_types:
            # neighbors_data_st[edge_type] = list()  # 7.18
            neighbors_data_lower_upper[edge_type] = list()
            neighbors_future[edge_type] = list()
            # neighbors_data[edge_type] = list()  # 7.18
            ...
            for connected_node in connected_nodes:
                ...
                neighbors_data.append(neighbor_state)  # 7.18
                neighbors_data_st.append(neighbor_state_st)  # 7.18
                neighbors_data_lower_upper[edge_type].append(torch.tensor([lower, upper]))
                neighbors_future[edge_type].append(neighbor_future)
        neighbors_data_ = torch.stack(neighbors_data, 0)  # 7.18
        neighbors_data_st_ = torch.stack(neighbors_data_st, 0)  # 7.18
    # Robot
    ...
    # 7.18
    return (
    first_history_index, x_t, y_t, x_st_t, y_st_t, neighbors_data_, neighbors_data_st_, neighbors_data_lower_upper,
    neighbors_future,
    neighbors_edge_value, robot_traj_st_t, map_tuple, scene.name, t)
